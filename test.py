...
classNames = ["person", "bicycle", "car", "motorbike", "aeroplane", "bus", "train", "truck", "boat",
              ...
              "baseball glove", "skateboard", "surfboard", "tennis racket", "milk", "wine glass", "cup",
...
def resize_image(image, width, height, interpolation=cv2.INTER_LINEAR):
    ...
    - interpolation: Interpolation method to use (default is cv2.INTER_LINEAR).
...
def detect_objects(image_path, model_path, csv_file_path, output_image_path,confidence_threshold):
    # Load the YOLO model
    model = YOLO(model_path)

    # image = get_image_from_ip_camera(url)
    image = get_image_from_directory(image_path)
    ...
    for r in results:
        #boxes = r.boxes
        filtered_boxes = [box for box in r.boxes if box.conf[0] >= confidence_threshold]  # Filter boxes based on confidence threshold
...
if __name__ == "__main__":
    ...
    image_path = "demo.jpg"
    model_path = "yolov8n.pt"  # Adjust the path as needed
